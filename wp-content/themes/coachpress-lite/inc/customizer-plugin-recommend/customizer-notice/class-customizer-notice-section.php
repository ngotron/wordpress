<?php
/**
 * CoachPress Lite Customizer Notification Section Class.
 *
 * @package CoachPress_Lite
 */

/**
 * CoachPress_Lite_Customizer_Notice_Section class
 */
class CoachPress_Lite_Customizer_Notice_Section extends WP_Customize_Section {
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'coachpress_lite_cust_notice_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'coachpress_lite_cust_notice_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		foreach ( $coachpress_lite_recommended_plugins as $slug => $plugin_opt ) {
			...
			$coachpress_lite_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$coachpress_lite_recommended_plugin['class'] = 'active';
			} else {
				$coachpress_lite_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$coachpress_lite_recommended_plugin['button_class'] = 'install-now button';
					$coachpress_lite_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$coachpress_lite_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$coachpress_lite_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$coachpress_lite_recommended_plugin['button_class'] = 'deactivate-now button';
					$coachpress_lite_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
            
			$info   = $this->call_plugin_api( $slug );
			$coachpress_lite_recommended_plugin['id'] = $slug;
			$coachpress_lite_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$coachpress_lite_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$coachpress_lite_recommended_plugin['description'] = $info->short_description;
			}

			$coachpress_lite_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $coachpress_lite_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="coachpress-lite-recommeded-actions-container coachpress-lite-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="coachpress-lite-recommeded-actions">
