jQuery(document).ready(function($){
    ...
    $( 'input[name=executive-coach-flush-local-fonts-button]' ).on( 'click', function( e ) {
        var data = {
            wp_customize: 'on',
            action: 'coachpress_lite_flush_fonts_folder',
            nonce: coachpress_lite_cdata.flushFonts
        };  
        $( 'input[name=executive-coach-flush-local-fonts-button]' ).attr('disabled', 'disabled');

        $.post( ajaxurl, data, function ( response ) {
            if ( response && response.success ) {
                $( 'input[name=executive-coach-flush-local-fonts-button]' ).val( 'Successfully Flushed' );
            } else {
                $( 'input[name=executive-coach-flush-local-fonts-button]' ).val( 'Failed, Reload Page and Try Again' );
            }
        });
    }); 
});
